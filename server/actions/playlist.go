...
func (a *Actions) DownloadPlaylist(playlistPubId string, profileId uint) (string, error) {
	...
	outFile, err := os.Create(fmt.Sprintf("%s/%s.zip", config.Env().BlobsDir, playlist.Title))
	if err != nil {
		return "", err
	}
...
func (a *Actions) DeletePlaylistArchive(event events.PlaylistDownloaded) error {
	...
	err := os.Remove(fmt.Sprintf("%s/playlists/%s.zip", config.Env().CdnAddress, event.PlaylistTitle))
	if err != nil {
		return err
	}
